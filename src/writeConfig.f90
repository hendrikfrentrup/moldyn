Subroutine writeConfig( actualStep, timeNow, str, trajOption, switch )
  ...
  ! Purpose: Write configurations or history file
  ...
  integer :: i, j, k, l
  integer, intent(in) :: actualStep, switch, trajOption
    ...
    do j=1,str%nSpecies
      ...
      do k=1, str%nMol(j)
        ...
        do l=1, str%nSites(j)
          ...
          if( trajOption == 0 ) then
            write(unit=1,fmt="(a3,3(1x,f15.8))")  str%nameSites(j), 10.0*str%rPart(:,i)
          else if( trajOption == 1 ) then
            write(unit=1,fmt="(a3,6(1x,f15.8))")  str%nameSites(j), 10.0*str%rPart(:,i), str%vPart(:,i)
          else
            write(unit=1,fmt="(a3,9(1x,f15.8))")  str%nameSites(j), 10.0*str%rPart(:,i), str%vPart(:,i), str%fPart(:,i)
          end if
